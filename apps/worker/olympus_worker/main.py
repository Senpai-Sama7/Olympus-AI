...
AUTO_CONSENT = os.getenv("OLY_AUTO_CONSENT", "false").lower() in (
    "1",
    "true",
    "yes",
)  # dev convenience
...
class ToolRegistry:
    def __init__(self):
        ...
        self.register(
            "fs.search", self._fs_search, scopes=["search_fs"]
        )  # custom scope
        self.register(
            "shell.run", self._shell_run, scopes=["exec_shell"]
        )  # custom scope
        self.register(
            "git.status", self._git_status, scopes=["git_ops"]
        )  # custom scope
        self.register("git.add", self._git_add, scopes=["git_ops"])  # custom scope
        self.register(
            "git.commit", self._git_commit, scopes=["git_ops"]
        )  # custom scope
    ...
    # ---- implementations ----
    def _fs_read(
        self, args: Dict[str, Any], consent: Optional[fstool.ConsentToken]
    ) -> Dict[str, Any]:
    ...
    def _fs_delete(
        ...
        return fstool.delete_path(
            args["path"], recursive=args.get("recursive", False), token=consent
        )
    ...
    def _http_get(
        ...
        if fstool.REQUIRE_CONSENT:
            if consent is None or (
                "*" not in consent.scopes and "net_get" not in consent.scopes
            ):
...
class PlanExecutor:
    def __init__(
        self, db: Optional[MemoryDB] = None, registry: Optional[ToolRegistry] = None
    ):
    ...
    async def _run_step(
        self, plan: Plan, step: Step, consent: Optional[fstool.ConsentToken]
    ) -> None:
        step.attempts += 1
        step.mark_running()
            ...
            try:
                ...
                self._emit(
                    PlanEvent(
                        ...
                        plan_id=plan.id,
                        step_id=step.id,
                        payload={"attempt": attempt, "output": out},
                    )
                )
                self._persist_plan(plan)
                return
                ...
                # deadline check
                if (
                    step.guard.deadline_ms is not None
                    and now_ms() - start > step.guard.deadline_ms
                ):
                    break
    ...
    async def run(
        self, plan: Plan, consent: Optional[fstool.ConsentToken] = None
    ) -> Plan:
        ...
        self._emit(
            PlanEvent(
                type="plan.started", plan_id=plan.id, payload={"title": plan.title}
            )
        )
            ...
            if failed:
                ...
                self._emit(
                    PlanEvent(
                        type="plan.failed",
                        plan_id=plan.id,
                        payload={"failed_steps": [f.id for f in failed]},
                    )
                )
                return plan
            if plan.all_done():
    ...
    async def run_by_id_with_consent(
        self, plan_id: str, consent: Optional[fstool.ConsentToken]
    ) -> Plan:
        row = self.db.get_plan(plan_id)
        if not row:
            raise RuntimeError(f"Plan {plan_id} not found")
        ...
        p = Plan(
            ...
            steps=[
                Step(
                    ...
                    input={"path": "demo/out.txt", "content": "hello"},
                ),
                Step(
                    name="read",
                    capability=cap_read,
                    input={"path": "demo/out.txt"},
                    deps=[],
                ),
            ],
        )
