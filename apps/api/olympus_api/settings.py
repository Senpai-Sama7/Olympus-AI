...
class Settings(BaseModel):
	...
	# CORS
	DEV_ALLOWED_ORIGINS: List[str] = Field(default_factory=lambda: ["http://localhost:3000", "http://127.0.0.1:3000"])
	...
	OLLAMA_MODEL_ALLOWLIST: List[str] = Field(default_factory=lambda: ["llama3:8b", "llama3.1:8b"])
	...
	with _lock:
		...
		try:
			settings = Settings(
				...
				DEV_ALLOWED_ORIGINS=_parse_list(os.environ.get("DEV_ALLOWED_ORIGINS")) or [
					"http://localhost:3000",
					"http://127.0.0.1:3000",
				],
				PROD_ALLOWED_ORIGINS=_parse_list(os.environ.get("PROD_ALLOWED_ORIGINS")),
				CORS_ALLOWED_METHODS=_parse_list(os.environ.get("CORS_ALLOWED_METHODS")) or [
					"GET",
					"POST",
					"PUT",
					"DELETE",
				...
				CORS_ALLOWED_HEADERS=_parse_list(os.environ.get("CORS_ALLOWED_HEADERS")) or [
					"Authorization",
					"Content-Type",
					"X-Request-ID",
				],
				CORS_MAX_AGE=_coerce_positive_int(os.environ.get("CORS_MAX_AGE"), 600),
				REQUEST_TIMEOUT_SEC=_coerce_positive_int(os.environ.get("REQUEST_TIMEOUT_SEC"), 30),
				CONNECT_TIMEOUT_SEC=_coerce_positive_int(os.environ.get("CONNECT_TIMEOUT_SEC"), 10),
				MAX_BODY_BYTES=_coerce_positive_int(os.environ.get("MAX_BODY_BYTES"), 5_000_000),
				RATE_LIMIT_GLOBAL_PER_MIN=_coerce_positive_int(os.environ.get("RATE_LIMIT_GLOBAL_PER_MIN"), 120),
				RATE_LIMIT_CHAT_PER_MIN=_coerce_positive_int(os.environ.get("RATE_LIMIT_CHAT_PER_MIN"), 30),
				SANDBOX_ROOT=os.environ.get("SANDBOX_ROOT", ".sandbox"),
				ALLOW_WRITE_DIRS=_parse_list(os.environ.get("ALLOW_WRITE_DIRS")) or [".sandbox"],
				DB_PATH=os.environ.get("DB_PATH", ".data/olympus.db"),
				WORKER_HEARTBEAT_PATH=os.environ.get("WORKER_HEARTBEAT_PATH", ".sandbox/.status/worker.json"),
				OLLAMA_BASE_URL=os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434"),
				OLLAMA_MODEL_ALLOWLIST=_parse_list(os.environ.get("OLLAMA_MODEL_ALLOWLIST")) or ["llama3:8b", "llama3.1:8b"],
				METRICS_ENABLED=os.environ.get("METRICS_ENABLED", "true").lower() in ("1", "true", "yes", "y"),
			)
