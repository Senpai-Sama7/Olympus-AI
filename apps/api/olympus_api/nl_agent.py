...
from .planner import propose_plan, TOOL_SCOPE
...
def _intent_prompt(
    user_text: str, allowed_tools: List[Dict[str, Any]]
) -> List[Dict[str, str]]:
    return [
...
async def handle_chat_turn(
    ...
    try:
        intent = await router.chat(
            messages=_intent_prompt(user_text, allowed_tools),
            temperature=0.2,
            max_tokens=max_tokens,
        )
    except Exception:
        prompt = "\n".join(
            m.get("content", "") for m in _intent_prompt(user_text, allowed_tools)
        )
    ...
    if action == "ask":
        return (
            {"reply": message or "I need more information.", "requires_input": True},
            None,
        )
        ...
        return (
            {
                ...
                "proposed_plan": {
                    "title": plan.title,
                    "steps": [
                        {"name": s.name, "capability": s.capability.name}
                        for s in plan.steps
                    ],
                },
            },
