...
class BackendManager:
	...
	def status(self) -> dict:
		...
		try:
			...
			health = {"code": r.status_code, "body": r.json() if r.headers.get("content-type",""").startswith("application/json") else r.text}
...
"""
		...
		proc = psutil.Popen([sys.executable, "-c", code], stdout=sys.stdout, stderr=sys.stderr)
		return proc
	...
	def _alive(proc: Optional[psutil.Popen]) -> bool:
		return bool(proc and proc.is_running() and proc.status() != psutil.STATUS_ZOMBIE)
