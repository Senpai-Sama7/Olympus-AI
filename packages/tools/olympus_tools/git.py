...
def git_status(workdir: str = "/", token: Optional[ConsentToken] = None) -> Dict:
    ...
    return run_shell_command(["git", "status", "--porcelain=v1"], workdir=workdir, token=t)


def git_add(paths: List[str], workdir: str = "/", token: Optional[ConsentToken] = None) -> Dict:
    t = _tok_with_scope(token, GIT_SCOPE)
    return run_shell_command(["git", "add", "--"] + list(paths), workdir=workdir, token=t)


def git_commit(message: str, workdir: str = "/", token: Optional[ConsentToken] = None) -> Dict:
    t = _tok_with_scope(token, GIT_SCOPE)
